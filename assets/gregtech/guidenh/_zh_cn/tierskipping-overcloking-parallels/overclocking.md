...
**超频**(Overclocking)指增加 [额定功率](T-O-P-index.md#额定功率)，使机器以更短的时间执行配方。机器的额定功率每超过 (实际并行数 × 配方实际功率) 一定倍率（通常是四倍），机器就发生一次超频，使配方时间缩短。与 **[升压](tierskipping.md)** 不同，超频可以多次发生。

超频与 [电压等级](T-O-P-index.md#电压等级) 没有关联。使用更高电压等级的能源仓可提升单能源仓额定功率，发生更多次超频；堆叠多个能源仓也可达到类似效果——安装两个同等级能源仓的常见做法称为**双仓超频**。
